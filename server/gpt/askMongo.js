...
async function readEmbeddingsFromCourse(courseId) {
  return await EmbeddingCache.getAllByCourse(courseId);
}
...
async function stringsRankedByRelatedness(query, course, topN = 100) {
  ...
  try {
    ...
    embeddings = await readEmbeddingsFromCourse(course._id);
  } catch (error) {
  ...
  const relatednessData = embeddings.map(embeddingRecord => {
    try {
      const embedding = JSON.parse(embeddingRecord.embedding);
