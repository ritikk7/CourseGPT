...
// async function stringsRankedByRelatedness(query, df, relatednessFn = (x, y) => 1 - scipy.spatial.distance.cosine(x, y), topN = 100) {
...
// async function ask(query, df = df, model = GPT_MODEL, tokenBudget = 4096 - 500, printMessage = false) {
...
//     const response = await openai.createChatCompletion({ model: model, messages: messages, temperature: 0 });
